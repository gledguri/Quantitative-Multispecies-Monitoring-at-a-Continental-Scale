...
\node (qPCR_st) [node, fill=orange!35] at (-4,-2) {
    $\begin{array}{c}
       % \mu_{ik} = \beta_{0p} + \beta_{1p}  K_{i} \\
        Z_{qpr} \sim \mathrm{Bernoulli} \left( 1 - \exp(-2 e^{K_{q}} \phi) \right) \\
        Y_{qpr} \sim \mathrm{N} (\beta_{0p} + \beta_{1p}  K_{q} , \sigma_{Yq}) \quad \textnormal{if } Z_{qpr} = 1 \\
        \sigma_{Yq} = \exp(\gamma_0 + \gamma_1  K_{q}) \\
    \end{array}$
};
...
\node (qPCR_en) [node, below=of qPCR_st, fill=orange!35]  at (0,-3) {
    $\begin{array}{c}
       % \mu_{ip} = \beta_{0p} + \beta_{1p} E_{Hi} \\
         Z_{ipr} \sim \mathrm{Bernoulli} \left( 1 - \exp(-2 e^{E_{Hji}} \phi) \right) \\
         Y_{ipr} \sim \mathrm{N} (\beta_{0p} + \beta_{1p} E_{Hji} , \sigma_{Yi}) \quad \textnormal{if } Z_{ipr} = 1 \\
...
\node (metabarcoding) [node, below=of state2s, fill=orange!35]  at (8,-3) {
    $\begin{array}{c}
        \nu_{si} = (E_{sji} - E_{Rji} ) + N_{PCR}  \alpha_s  \\  [8pt]

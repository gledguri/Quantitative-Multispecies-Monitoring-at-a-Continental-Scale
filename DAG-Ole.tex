...
\usepackage[paperwidth=250mm, paperheight=200mm, margin=0cm]{geometry}
\usepackage{tikz}
...
\node (qPCR_st) [node, fill=orange!35] at (-4,-2) {
    $\begin{array}{c}
       % \mu_{ik} = \beta_{0p} + \beta_{1p}  K_{i} \\
        \sigma_{Yj} = \exp(\gamma_0 + \gamma_1  K_{j}) \\
        Z_{jpr} \sim \mathrm{Bernoulli} \left( 1 - \exp(-2 e^{K_{j}} \phi) \right) \\
        Y_{jpr} \sim \mathrm{N} (\beta_{0p} + \beta_{1p}  K_{j} , \sigma_{Yj}) \quad \textnormal{if } Z_{jpr} = 1 \\
    \end{array}$
};
...
\node (qPCR_en) [node, below=of qPCR_st, fill=orange!35]  at (0,-3) {
    $\begin{array}{c}
       % \mu_{ip} = \beta_{0p} + \beta_{1p} E_{Hi} \\
        \sigma_{Yi} = \exp(\gamma_0 + \gamma_1 E_{Hi})\\
         Z_{ipr} \sim \mathrm{Bernoulli} \left( 1 - \exp(-2 e^{E_{Hi}} \phi) \right) \\
         Y_{ipr} \sim \mathrm{N} (\beta_{0p} + \beta_{1p} E_{Hi} , \sigma_{Yi}) \quad \textnormal{if } Z_{ipr} = 1
    \end{array}$
};
...
% + \delta1_{ijdb}
    $\begin{array}{c}
        ...
        D_{H}(x,y,d) = \psi  + \epsilon({x,y,d}) \\
        \epsilon({x,y,d})\sim \mathrm{N}(0,\tau)
    \end{array}$
};
...
\node (state2H) [node, above=of qPCR_en, fill=myBlue] at (2.5,-2.5){ 
% + \delta0_{ijdbt} 
% + \delta1_{ijdb}
    $\begin{array}{c}
        %\ln C_{ijdbt} = \psi_i + \ln D_{ijd} + \rho_{ijdbt} + \omega_{ijdbt} + \xi_{ijdbt} \\[8pt]
        %\ln C_{ijdbt} = \psi_i + \ln D_{ijd} + \rho_{ijdbt} + W_{ijdbt} + \xi_{ijdbt} \\[8pt]
       E_{Hi}(x,y,d) = D_H({x,y,d}) +  \log{V_i} + \\
       		\qquad \qquad \qquad \qquad \log{I_i} + \mathbf{I}\zeta + \delta_i(x,y,d) \\
		\delta_i(x,y,d) \sim \mathrm{N}(0,\sigma_D) \qquad \qquad
    \end{array}$
};
...
\node (state2s) [node, right=of state2H, fill=myBlue]at (4.5,0) { 
% + \delta0_{ijdbt} 
% + \delta1_{ijdb}
    $\begin{array}{c}
        %\ln C_{ijdbt} = \psi_i + \ln D_{ijd} + \rho_{ijdbt} + \omega_{ijdbt} + \xi_{ijdbt} \\[8pt]
...
\node (metabarcoding) [node, below=of state2s, fill=orange!35]  at (8,-3) {
    $\begin{array}{c}
        \nu_{si} = (E_{si} - E_{Ri} ) + N_{PCR}  \alpha_s  \\  [8pt]
        \pi_{si} = \frac{exp({\nu_{si}})}{\sum_{s=1}^{S} exp({\nu_{si}})} \\ [8pt]
         \mathbf{R}_{i} \sim \mathrm{Multinomial} \left( \boldsymbol{\pi}_{i}, T_{i} \right) 
    \end{array}$
};
...
\node (mock) [node,  right=of metabarcoding,fill=yellow!50] at (9,-2.75){
    $\begin{array}{c}
       \nu_{sm} = (L_{sm} - L_{Rm}) + N_{PCR}  \alpha_s     \\   [8pt]
       \pi_{sm} = \frac{exp({\nu_{sm}})}{\sum_{s=1}^{S} exp({\nu_{sm}})} \\ [8pt]
       \mathbf{R}_{m} \sim \mathrm{Multinomial} \left( \boldsymbol{\pi}_{m}, T_{m} \right) 
    \end{array}$
};
...
\node (spatial) [node, right=of state2s, fill=green!40] at (8,2){
    $\begin{array}{c}
        F_s(x,y,d) = s\left(E_{s\neq H}(x,y,d) \right)
    \end{array}$
};
...
\node (parameters) [rectangle, draw, align=left, below=of metabarcoding, text width=9cm, fill=myGrey] at (3.5,-6){
   ...
   % $\mu$ = mean Ct values of qPCR \\
    ...
    $\gamma0, \gamma1$ = intercept and slope defining  standard deviation of qPCR - DNA concentration relationship \\
...
% Legend: Data
\node (States of Interest) [rectangle, draw, align=left, below=of subscripts, text width=8cm, fill=myGrey] at (12.5,-10){
    \textbf{States } \\

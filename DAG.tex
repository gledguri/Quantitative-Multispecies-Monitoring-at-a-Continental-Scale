...
\node [above=of mock, yshift=-1.5cm, font=\large\bfseries] {Mock metabarcoding model};
...
\node (parameters) [rectangle, draw, align=left, right=of qPCR_en, text width=12cm, fill=myGrey] at (11.9,-0.8){
    ...
    $\pi$ = sequence proportions at the end of PCR [0,1] \\
    $\nu$ = sequence proportions at the end of PCR \\
    $\tau$ = scalar intercept\\
    $\eta$ = spatial field \\
    $\varepsilon$ =  depth specific spatial field
};
% Legend: Subscripts
\node (subscripts) [rectangle, draw, align=left, below=of qPCR_st, text width=7cm, fill=myGrey] at (0.4,0) {
    \textbf{Subscripts} \\
    $i$ = species \\
    $j$ = station \\
    $d$ = depth \\
...
\node (data) [rectangle, draw, align=left, below=of subscripts, text width=7cm, fill=myGrey] at (0.4,-5.6){
    \textbf{Data} \\
    $Z$ = qPCR amplification yes/no (0,1) \\
    $Y$ = qPCR Ct values \\
...
\node (state) [rectangle, draw, align=left, right=of qPCR_en, text width=6cm, fill=myGrey] at (11.9,-9){
    \textbf{State variables} \\
    $C$ = unknown eDNA concentration \\
    $D$ = unknown eDNA concentration
};
    \end{tikzpicture}
